...
- **수치 가드(정확도에만 건다, 밀도에 걸지 않는다)**: gpt-image-2는 데이터를 플롯하지 못한다 — "38% 막대"는 그럴듯한 비율의 장식 막대로 렌더된다. 수치가 메시지면 **숫자를 큰 타이포로 렌더**하고 도형은 보조 장식으로. **정확히 렌더돼야 하는 카피(헤드라인·섹션 헤더·핵심 수치)만 8개 이내 따옴표 고정**, 셀 내부 디테일 문단은 `small descriptive caption text` 로만 지시(오탈자 허용 존 — 밀도 연출용, 철칙 예외 아님: 실제 문구를 안 박는 것). 교차 화살표·다대다 연결은 뭉개진다 — 단방향 플로우·사이드 카드·리더선으로 단순화.
