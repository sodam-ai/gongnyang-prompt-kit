...
도표·인포그래픽은 **수치 가드** 주의(C6 보충) — 모델은 데이터를 플롯하지 못한다. 상한은 밀도가 아니라 정확도에: **정확 카피(헤드라인·섹션 헤더·핵심 수치)만 8개 이내 따옴표 고정**, 디테일 문단은 `small descriptive caption text`로 밀도만 채운다(오탈자 허용 존). 정밀 차트는 이미지 밖(코드 렌더) 권고. C6·C7은 **밀도가 기본값** — 미니멀 플로우·매거진 여백형은 요청이 명시할 때만(실측: 나이브 고밀도 컷에 짐).
